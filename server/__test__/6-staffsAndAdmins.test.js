...
describe('Testing Staff and Admin Controller for endpoints that only both are privileged to use', () => {
  ...
  describe('Testing view all current bank accounts controller', () => {
    const viewAllCurrentAccountsUrl = '/api/v1/accounts?type=current';
    describe('Testing view all current bank accounts controller for Logged-in Admin', () => {
      it('should return all current bank accounts if admin is authenticated', (done) => {
        chai.request(app)
          .get(viewAllCurrentAccountsUrl)
          .set('authorization', `Bearer ${adminSignInToken}`)
          .end((err, res) => {
            expect(res).to.have.status(200);
            expect(res.body).to.be.an('object');
            ...
            expect(res.body.data[0]).to.have.property('ownerEmail');
            expect(res.body.data[0]).to.have.property('type');
            expect(res.body.data[0].type).to.equal('current');
            expect(res.body.data[0]).to.have.property('status');
            expect(res.body.data[0]).to.have.property('balance');
            done();
          });
    ...
    describe('Testing view all current bank accounts controller for Logged-in Staff', () => {
      it('should return all bank current accounts if cashier is authenticated', (done) => {
        chai.request(app)
          .get(viewAllCurrentAccountsUrl)
          .set('authorization', `Bearer ${staffSignInToken}`)
          .end((err, res) => {
    ...
    describe('Testing view all current bank accounts controller for a non admin nor staff sign-in', () => {
      let clientToken;
      const clientSignInUrl = '/api/v1/auth/signin';
      it('should not return bank accounts', (done) => {
        chai.request(app)
